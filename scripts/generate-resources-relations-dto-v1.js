/**
 * ================================================================
 * Script: generate-resources-typed.js (con manejo automático de relaciones)
 * Descripción:
 *  - Genera recursos CRUD en NestJS desde Prisma.
 *  - Incluye `include` automáticamente solo si el modelo tiene relaciones.
 *  - No afecta modelos sin relaciones (ej. archivo).
...
while ((match = modelRegex.exec(schema)) !== null) {
  ...
  for (const line of lines) {
    ...
    fields.push({
      ...
      type,
      optional: optQ === "?",
      omitInCreate: isId || /@default\s*\(\s*now\(\)\s*\)/.test(attrs) || /@default\s*\(\s*autoincrement\(\)\s*\)/.test(attrs),
    });
  }
  ...
  fs.writeFileSync(path.join(folderPath, "dto", `create-${fileName}.dto.ts`), createDto);
...
`;
  fs.writeFileSync(path.join(folderPath, "dto", `update-${fileName}.dto.ts`), updateDto);
...
`;
  ...
  console.log(`✅ Generated resources for model: ${modelName} (relations: ${relations.join(", ") || "none"})`);
}
